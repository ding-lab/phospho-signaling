...
for (diffexp_dir in c("up")) {
  ...
  t0 <- t0[t0$known_enzyme_phosphosite & !is.na(t0$known_enzyme_phosphosite),]
  ...
  if (nrow(t0) > 0){
    ...
    if (TRUE){
      ...
      plot3 = plot3 + geom_text(data = t2[t2$Freq > 0,], mapping = aes(x=Cancer, y=GENE, label = Freq), size = 5)
      plot3 = plot3 + theme_bw() + theme_nogrid()
      plot3 = plot3 + theme(axis.title = element_blank(), axis.text.y = element_text(size = 12), strip.text.y = element_text(size = 5))
      plot3 = plot3 + theme(axis.text.x = element_text(size = 12))
      ...
      fn = paste(makeOutDir(resultD = resultD), "down", '_', enzyme_type ,'_yp_substrate_pairs_number.pdf',sep ="")
      plot3
...
for (diffexp_dir in c("down")) {
  ...
  if (nrow(t0) > 0){
    ...
    if (TRUE){
      ...
      plot3 = plot3 + theme(axis.ticks=element_blank(), legend.position="bottom")
      plot3 = plot3 + theme(panel.spacing.x=unit(0, "lines"), panel.spacing.y=unit(0, "lines"))
      fn = paste(makeOutDir(resultD = resultD), "up", '_', enzyme_type ,'_down_substrate_pairs_number.pdf',sep ="")
      plot3
      ggsave(filename = fn, width = 3, height = 4)
    } else {
      print('not enough data')
