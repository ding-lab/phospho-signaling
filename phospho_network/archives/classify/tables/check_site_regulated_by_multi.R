...
cis_tab <- fread(paste0(resultD, "phospho_network/kinase_activity/tables/integrate_enzyme_KSEA_diffexp_sub_FC_plus_cistrans_regression_plus_genoalt/", 
                        "3can", "_KSEA_enzyme_diffexp_substrate_aa_reported.txt"), data.table = F)
cis_tab <- cis_tab[cis_tab$SELF == "cis",]
cis_sig_tab <- cis_tab[cis_tab$FDR_pro_kin.aa_reported < 0.1 & cis_tab$coef_pro_kin > 0,]
...
trans_pk_sig_tab <- trans_pk_tab[trans_pk_tab$FDR_pho_kin.aa_reported < 0.1 & trans_pk_tab$coef_pho_kin > 0,]
trans_pp_sig_tab <- trans_pp_tab[trans_pp_tab$FDR_pho_kin.aa_reported < 0.2 & trans_pp_tab$coef_pho_kin < 0,]
...
cis_tab <- sup_tab[!is.na(sup_tab$FDR_pro_kin.aa_reported),]
cis_sig_tab <- cis_tab[cis_tab$FDR_pro_kin.aa_reported < 0.1 & cis_tab$coef_pro_kin > 0,]
...
write.table(x = trans_pk_pp_sig_tab, file = "./cptac2p/analysis_results/phospho_network/classify/tables/c")
