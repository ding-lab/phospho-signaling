...
sup_cans_tab_pp <- fread(input = paste0("./cptac2p/analysis_results/phospho_network/regression/tables/regression_omnipath&newworkin&depod_protein_level/", enzyme_type, "_substrate_regression_cptac2p_3can_tumor.txt"), data.table = F)
sup_cans_tab_pp$pair <- paste0(sup_cans_tab_pp$KINASE, ":", sup_cans_tab_pp$SUBSTRATE, ":", sup_cans_tab_pp$SUB_MOD_RSD)
sup_cans_tab_pp$GENE <- as.vector(sup_cans_tab_pp$KINASE)
sup_cans_tab_pp$SUB_GENE <- as.vector(sup_cans_tab_pp$SUBSTRATE)
sup_cans_tab_pp <- markSigSiteCan(sup_cans_tab_pp, sig_thres = fdr_pp, enzyme_type = enzyme_type)

enzyme_type <- "kinase"
sup_cans_tab_pk <- fread(input = paste0("./cptac2p/analysis_results/phospho_network/regression/tables/regression_omnipath&newworkin&depod_protein_level/", enzyme_type, "_substrate_regression_cptac2p_3can_tumor.txt"), data.table = F)
sup_cans_tab_pk$pair <- paste0(sup_cans_tab_pk$KINASE, ":", sup_cans_tab_pk$SUBSTRATE, ":", sup_cans_tab_pk$SUB_MOD_RSD)
sup_cans_tab_pk$GENE <- as.vector(sup_cans_tab_pk$KINASE)
sup_cans_tab_pk$SUB_GENE <- as.vector(sup_cans_tab_pk$SUBSTRATE)
sup_cans_tab_pk <- markSigSiteCan(sup_cans_tab_pk, sig_thres = fdr_pk, enzyme_type = enzyme_type)
...
sup_cans_tab$pair_pro <- paste0(sup_cans_tab$GENE, ":", sup_cans_tab$SUB_GENE)
...
tmp <- data.frame(shared_name = sup_cans_tab2_trans4cytoscape$GENE,
                  target_type = "kinase", name_print = sup_cans_tab2_trans4cytoscape$GENE)
