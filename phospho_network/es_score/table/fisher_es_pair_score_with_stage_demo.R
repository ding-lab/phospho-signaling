...
for (i in 5) {
  ...
  is.highstage <- matrix(ccRCC_clinical$Tumor_Stage_Pathological %in% c("Stage III", "Stage IV"), nrow = 1)
  colnames(is.highstage) <- ccRCC_clinical$Case_ID
  
  fisher_stat_tmp <- sapply(rownames(esscore_tab_outlier), FUN = function(pair, is.highstage, outlier_df) {
    partIDs_nonNA <- colnames(outlier_df)[which(outlier_df[pair,] == TRUE | outlier_df[pair,] == FALSE)]
    partIDs_nonNA <- intersect(partIDs_nonNA, colnames(is.highstage))
    tab4fisher <- table(outlier_df[pair, partIDs_nonNA], is.highstage[,partIDs_nonNA])
    
    fisher_p.value <- NA; fisher_or_bottom <- NA; fisher_or_upper <- NA
    num_is.outlier_is.highstage <- NA
    num_not.outlier_not.subtype <- NA
    num_is.outlier_not.subtype <- NA
    num_not.outlier_is.highstage <- NA
    
    if (any(outlier_df[pair, partIDs_nonNA] & is.highstage[,partIDs_nonNA])) {
      if (length(as.vector(tab4fisher)) == 4) {
        ...
        num_is.outlier_is.highstage = as.vector(tab4fisher)[4]
        num_not.outlier_not.subtype = as.vector(tab4fisher)[1]
        num_is.outlier_not.subtype = as.vector(tab4fisher)[2]
        num_not.outlier_is.highstage = as.vector(tab4fisher)[3]
      } 
    }
    fisher_stat <- c(fisher_p.value, fisher_or_bottom, fisher_or_upper,
                     num_is.outlier_is.highstage, num_not.outlier_not.subtype,
                     num_is.outlier_not.subtype, num_not.outlier_is.highstage)
    return(fisher_stat)
  }, is.highstage = is.highstage, outlier_df = esscore_tab_outlier)
  
  fisher_stat_tab <- t(fisher_stat_tmp)
  colnames(fisher_stat_tab) <- c("p.value", "or_bottom", "or_upper", 
                                 "num_is.outlier_is.highstage", "num_not.outlier_not.highstage",
                                 "num_is.outlier_not.highstage", "num_not.outlier_is.highstage")
  ...
  fisher_stat_tab$clinical_info <- "stage_high"
  ...
  fn <- paste0(makeOutDir(resultD = resultD),
               "fisher_stat_tab_esscore_outlier", outlier_sd, "SD_highstage", cancer, "_", enzyme_type, "_reg_nonNA", reg_nonNA, ".txt")
