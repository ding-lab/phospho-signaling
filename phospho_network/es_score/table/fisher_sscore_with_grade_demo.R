...
regression %>%
  filter(KINASE == "PRKAA1") %>%
  filter(SUBSTRATE == "ACACA") %>%
  filter(SUB_MOD_RSD == "S117")
...
for (i in 5) {
  ...
  fn <- paste0(makeOutDir(resultD = resultD),
               "fisher_stat_tab_outlier", outlier_sd, "SD_highgrade", cancer, "_", enzyme_type, "_reg_nonNA", reg_nonNA, ".txt")
