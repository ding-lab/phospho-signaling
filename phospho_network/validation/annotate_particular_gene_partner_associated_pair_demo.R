...
regression <- fread(input = paste0(ppnD, "regression/tables/run_regression_protein_level/", 
                                   "regression_cptac3_CCRCC_tumor_PGDAC_MD_MAD.txt"),
                    data.table = F)
regression %>% nrow()
...
regression2merge_ex2 <- regression %>%
  filter(FDR_pho_kin < 0.2 & coef_sig) %>%
  filter(GENE %in% pair_tab_annotated_gene$SUB_GENE | SUB_GENE %in% pair_tab_annotated_gene$SUB_GENE) %>%
  filter(Cancer == "CCRCC") %>%
  mutate(phosphosite = paste0(SUB_GENE, ":", SUB_MOD_RSD))
