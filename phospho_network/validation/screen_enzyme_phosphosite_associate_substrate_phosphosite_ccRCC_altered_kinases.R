...
# Filter regulatory or disease related phosphosites -----------------------
...
regression2check <- regression %>%
  filter(Cancer == "CCRCC") %>%
  filter(regulated == T) %>%
  filter(SUB_phosphosite.is_regulatory | SUB_phosphosite.is_disease_related | is.direct)
...
regression2check <- regression %>%
  filter(Cancer == "CCRCC") %>%
  filter(regulated == T) %>%
  filter(ENZ_phosphosite.is_regulatory | ENZ_phosphosite.is_disease_related | SUB_phosphosite.is_regulatory | SUB_phosphosite.is_disease_related | is.direct)
