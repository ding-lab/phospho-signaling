...
for (cancer in c("BRCA", "OV", "CO")) {
  ...
  trans_pk_count <- data.frame(table(unique(trans_pk_sig_tab[, c("GENE", "site")])[, "GENE"]))
  ...
  for (i in 1:(top_num-1)) {
    ...
    for (j in (i+1):top_num) {
      ...
      if (length(which((trans_pk_sig_tab$GENE == ki & trans_pk_sig_tab$SUB_GENE == kj))) > 0){
        count <- count + 1
        regulated[count] <- TRUE
        kinase1[count] <- ki
        kinase2[count] <- kj
        shared_num[count] <- length(shared_sites) + 1
      } 
      if (length(which((trans_pk_sig_tab$GENE == kj & trans_pk_sig_tab$SUB_GENE == ki))) > 0){
        count <- count + 1
        regulated[count] <- TRUE
        kinase1[count] <- kj
        kinase2[count] <- ki
      ...
      if (length(which((trans_pk_sig_tab$GENE == kj & trans_pk_sig_tab$SUB_GENE == ki) | (trans_pk_sig_tab$GENE == ki & trans_pk_sig_tab$SUB_GENE == kj))) == 0) {
        count <- count + 1
  ...
  shared_tab <- shared_tab[!is.na(shared_tab$kinase2),]
  shared_tab <- unique(shared_tab)
  write.table(x = shared_tab, file = paste0(makeOutDir(resultD = resultD), cancer, "_shared_site_sig", sig_pk, "_top", top_num, ".txt"), 
              quote = F, row.names = F, sep = "\t")
  write.table(x = trans_pk_count, file = paste0(makeOutDir(resultD = resultD), cancer, "_trans_pk_count_sig", sig_pk, "_top", top_num, ".txt"), 
              quote = F, row.names = F, sep = "\t")
}
