...
# plot the patients showing all KS phospho outlier--------------------------------------------------------------------
...
### order by phosphosites by associated kinase phosphosite
phosphosites_in_pairs_outlier <- lapply(X = unique(ks_result_tmp$ENZ_phosphosite), FUN = function(p_enz, ks_tab) {
  p_subs <- ks_tab$SUB_phosphosite[ks_tab$ENZ_phosphosite == p_enz]
  result_tmp <- c(p_enz, p_subs)
  return(result_tmp)
}, ks_tab = ks_result_tmp)
phosphosites_in_pairs_outlier <- unlist(phosphosites_in_pairs_outlier)
## filter the phosphosite outlier status result for these phosphosites
rownames(phosphosite_outlier_df) <- phosphosite_outlier_df$gene_phosphosite
plot_df <- phosphosite_outlier_df[phosphosites_in_pairs_outlier,]
## group the substrate phosphosites by kinase phosphosite
plot_df$kinase_phosphosite_group <- unlist(lapply(X = unique(ks_result_tmp$ENZ_phosphosite), FUN = function(p_enz, ks_tab) {
  p_subs <- ks_tab$SUB_phosphosite[ks_tab$ENZ_phosphosite == p_enz]
  result_tmp <- rep(p_enz, (length(p_subs) + 1))
  return(result_tmp)
}, ks_tab = ks_result_tmp))
...
gaps_row_vec <- table(plot_df$kinase_phosphosite_group)
gaps_row_vec <- gaps_row_vec[unique(plot_df$kinase_phosphosite_group)]
gaps_row_vec <- cumsum(gaps_row_vec)
## plot heatmap
library(pheatmap)
...
save_pheatmap_pdf(x = my_heatmap, 
                  filename = paste0(dir_out, "Heatmap_Patients_Outlier_Status_in_All_KS_Pairs.", run_id, ".pdf"), 
                  width = 12, height = 18)

# plot the patients showing selected KS phospho outlier--------------------------------------------------------------------
## filter for top 10 KS pairs showing the highest number of patients showing outlier KS
ks_result_tmp <- ks_result_path_tmp %>%
  filter(pair %in% pairs_outlier) %>%
  arrange(-(ES_phosphosite.num_outlier)) %>%
  mutate(GENE_SUB_GENE = paste0(GENE, "_", SUB_GENE)) %>%
  distinct(GENE_SUB_GENE, .keep_all = T)
## for KS pairs showing outlier, get all involving phosphosite
### order by phosphosites by associated kinase phosphosite
phosphosites_in_pairs_outlier <- lapply(X = unique(ks_result_tmp$ENZ_phosphosite), FUN = function(p_enz, ks_tab) {
  p_subs <- ks_tab$SUB_phosphosite[ks_tab$ENZ_phosphosite == p_enz]
...
phosphosites_in_pairs_outlier <- unlist(phosphosites_in_pairs_outlier)
## filter the phosphosite outlier status result for these phosphosites
rownames(phosphosite_outlier_df) <- as.vector(phosphosite_outlier_df$gene_phosphosite)
plot_df <- phosphosite_outlier_df[phosphosites_in_pairs_outlier,]
## group the substrate phosphosites by kinase phosphosite
plot_df$kinase_phosphosite_group <- unlist(lapply(X = unique(ks_result_tmp$ENZ_phosphosite), FUN = function(p_enz, ks_tab) {
  p_subs <- ks_tab$SUB_phosphosite[ks_tab$ENZ_phosphosite == p_enz]
...
save_pheatmap_pdf(x = my_heatmap, 
                  filename = paste0(dir_out, "Heatmap_Patients_Outlier_Status_in_Selected_KS_Pairs.", run_id, ".pdf"), 
                  width = 12, height = 11)
