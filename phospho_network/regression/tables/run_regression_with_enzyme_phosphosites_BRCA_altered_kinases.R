...
disease_sites <- disease_sites %>%
  mutate(SUB_MOD_RSD = str_split_fixed(string = MOD_RSD, pattern = "-", n = 2)[,1]) %>%
  mutate(phosphosite = paste0(GENE, "_", SUB_MOD_RSD)) %>%
  filter(grepl(x = DISEASE, pattern = "breast"))
...
kinases_genomics_altered <- kinase_driver_table %>%
  filter(grepl(x = `Mutational drivers tumor typed`, pattern = "BRCA") | grepl(x = `CNA cancer drivers tumor typed`, pattern = "BRCA") | grepl(x = `Oncogenic fusion drivers tumor typed`, pattern = "BRCA")) %>%
  select(`Gene symbol`)
