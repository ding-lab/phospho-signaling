...
datasets2process <- matrix(data = c("LUAD", "PGDAC", "tumor", "SD", "cptac3",
...
disease_sites <- disease_sites %>%
  mutate(SUB_MOD_RSD = str_split_fixed(string = MOD_RSD, pattern = "-", n = 2)[,1]) %>%
  mutate(phosphosite = paste0(GENE, "_", SUB_MOD_RSD)) %>%
  filter(grepl(pattern = "lung cancer", x = DISEASE) == T)

# input LUAD important genes ---------------------------------------------
genes2test <- unique(c("KEAP1", "NFE2L2", "NFE2L1"))
genes2test
...
for (i in 1:nrow(datasets2process)) {
  ...
  pairs2test_tab <- pairs2test_tab %>%
    filter(GENE %in% genes2test | SUB_GENE %in% genes2test)
  ...
  for (j in 1:npairs2test){
    ...
    df4regression <- data.frame(t(rbind(pho_sub,pho_enz)))
    colnames(df4regression) <- c("pho_sub","pho_enz")
    data_complete <- df4regression[complete.cases(df4regression),]
    size <- nrow(data_complete)
    if(size > least_samples ){
      fit2 <- lm(pho_sub ~ pho_enz, data = df4regression)
      
      # P_pro_sub[j] <- c(coef(summary(fit2))[2,4])
      # coef_pro_sub[j] <- fit2$coefficients[2]
      P_pho_kin[j] <- c(coef(summary(fit2))["pho_enz",4])
      coef_pho_kin[j] <- fit2$coefficients["pho_enz"]
      Size[j] <- size
  ...
  table_trans <- table_trans %>%
    filter(!is.na(Size) == T) %>%
    mutate(model = "pho_sub~pho_enz_site") %>%
