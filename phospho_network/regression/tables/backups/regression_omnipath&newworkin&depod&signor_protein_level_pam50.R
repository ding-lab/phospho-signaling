...
# manual addition of some pairs -------------------------------------------
ptms_pairs2add <- ptms_site_pairs_sup[ptms_site_pairs_sup$GENE == "PPP3CA" & ptms_site_pairs_sup$SUB_GENE == "NFATC1",]
ptms_pairs2add$GENE <- "PPP3CB"
ptms_site_pairs_sup <- rbind(ptms_pairs2add, ptms_site_pairs_sup)

## add from Stebbing, J., et al. "The regulatory roles of phosphatases in cancer." Oncogene 33.8 (2014): 939.
Stebbing2014 <- data.frame(GENE = c("PTPN13", "PTPN1", "PTPRT"), 
                           SUB_GENE = c("FAS", "SRC", "STAT3"), 
                           Cancer = c("CO", "CO", "CO"))
ptms_pairs2add <- ptms_site_pairs_sup[ptms_site_pairs_sup$GENE == "PPP3CA" & ptms_site_pairs_sup$SUB_GENE == "NFATC1",]
ptms_pairs2add$Source <- "Stebbing2014"
for (i in 1:nrow(Stebbing2014)) {
  ptms_pairs2add_tmp <- ptms_pairs2add
  ptms_pairs2add_tmp$GENE <- as.character(Stebbing2014$GENE[i])
  ptms_pairs2add_tmp$SUB_GENE <- as.character(Stebbing2014$SUB_GENE[i])
  ptms_site_pairs_sup <- rbind(ptms_pairs2add_tmp, ptms_site_pairs_sup)
}

## add from Sun, Tingting, et al. "Activation of multiple proto-oncogenic tyrosine kinases in breast cancer via loss of the PTPN12 phosphatase." Cell 144.5 (2011): 703-718.
Sun2011 <- data.frame(GENE = c("PTPN12", "PTPN12", "PTPN12"), 
                      SUB_GENE = c("EGFR", "ERBB2", "PDGFRB"), 
                      Cancer = c("BRCA", "BRCA", "BRCA"))
ptms_pairs2add <- ptms_site_pairs_sup[ptms_site_pairs_sup$GENE == "PPP3CA" & ptms_site_pairs_sup$SUB_GENE == "NFATC1",]
ptms_pairs2add$Source <- "Sun2011"
for (i in 1:nrow(Sun2011)) {
  ptms_pairs2add_tmp <- ptms_pairs2add
  ptms_pairs2add_tmp$GENE <- as.character(Sun2011$GENE[i])
  ptms_pairs2add_tmp$SUB_GENE <- as.character(Sun2011$SUB_GENE[i])
  ptms_site_pairs_sup <- rbind(ptms_pairs2add_tmp, ptms_site_pairs_sup)
}
