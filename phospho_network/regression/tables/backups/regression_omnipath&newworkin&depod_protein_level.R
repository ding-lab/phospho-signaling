...
for (sample_type in c("tumor")) {
  ...
  # for( enzyme_type in c("phosphatase")) {
    ...
    for (cancer in cancers2process) { 
      pro_data <- loadProteinNormalizedTumor(cancer = "UCEC")
      pho_data <- loadPhosphositeNormalizedTumor(cancer = "UCEC")
      pho_gdata <- loadPhosphoproteinNormalizedTumor(cancer = "UCEC")
    ...
    # write out tables --------------------------------------------------------
    # tn = paste0(makeOutDir(resultD = resultD), enzyme_type,"_substrate_regression_cptac2p_3can_", sample_type, ".txt")
    # write.table(table_3can, file=tn, quote=F, sep = '\t', row.names = FALSE)
    tn = paste0(makeOutDir(resultD = resultD), enzyme_type,"_substrate_regression_cptac3_", sample_type, ".txt")
    write.table(table_3can, file=tn, quote=F, sep = '\t', row.names = FALSE)
  }
}
