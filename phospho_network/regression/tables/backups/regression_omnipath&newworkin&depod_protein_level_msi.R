...
for (sample_type in c("tumor")) {
  ...
  # for( enzyme_type in c("phosphatase")) {
    ...
    for (cancer in c("CO")) { 
      ...
      msi <- sampID2MSI(sampleID_vector = samples, sample_map = loadSampMap(), subtype_map = msi_map)
      subtypes <- c("MSI-H", "MSI-L", "MSS")
      for (subtype in subtypes) { 
        samples_subtype <- samples[msi == subtype]
    ...
    # write out tables --------------------------------------------------------
    tn = paste0(makeOutDir(resultD = resultD), enzyme_type,"_substrate_regression_cptac2p_", cancer, "_msi_", sample_type, ".txt")
