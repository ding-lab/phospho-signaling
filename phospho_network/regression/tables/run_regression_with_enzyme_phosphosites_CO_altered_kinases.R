...
datasets2process <- matrix(data = c("CO", "CDAP", "tumor", "scaled", "cptac2p"), ncol = 5, byrow = T)
datasets2process
...
disease_sites <- disease_sites %>%
  mutate(SUB_MOD_RSD = str_split_fixed(string = MOD_RSD, pattern = "-", n = 2)[,1]) %>%
  mutate(phosphosite = paste0(GENE, "_", SUB_MOD_RSD)) %>%
  filter(grepl(x = DISEASE, pattern = "colorectal"))
...
kinases_genomics_altered <- kinase_driver_table %>%
  filter(grepl(x = `Mutational drivers tumor typed`, pattern = "COREAD") | grepl(x = `CNA cancer drivers tumor typed`, pattern = "COREAD") | grepl(x = `Oncogenic fusion drivers tumor typed`, pattern = "COREAD")) %>%
  select(`Gene symbol`)
...
## manually curated list by literature search
genes2test <- c("EGFR", "NTRK1", "AURKA", "ACVR2A", "ACVR1B", "TGFBR1", "TGFBR2", "ATM", "IGF1R", "ERBB2", "ERBB3", "BRAF", "NTRK2", "FGFR2", "RET", "ALK")
genes2test
