...
for (i in 1:nrow(data2process)) {
  ...
  pro_data <- loadRNA(cancer = cancer)
    ...
    if (nrow(pho_kinase_g) > 0 ){
      ...
      for (substrate in k_sub){# for each substrate for one kinase
        ...
        if(nrow(pro_sub) != 0){
          ...
          data2 <- data.frame(t(rbind(pho_sub,pho_kinase_g)))
          colnames(data2) <- c("pho_sub","pho_kinase_g")
          data_complete <- data2[complete.cases(data2),]
          size <- nrow(data_complete)
          
          if(size > least_samples ){
            fit2 <- lm(pho_sub ~ pho_kinase_g, data = data2)
            ...
            if (!any(is.na(fit2$coefficients))) {
              P_pho_kin[count] <- c(coef(summary(fit2))[2,4])
              coef_pho_kin[count] <- fit2$coefficients[2]
            }
            Size[count] <- size
  ...
  table_trans$model <- "mRNA_sub~mRNA_kin"
