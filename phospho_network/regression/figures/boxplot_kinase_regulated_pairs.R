  ...
  for (reg_nonNA in reg_nonNA2test) {
    ...
    for (fdr_thres in fdr_thres2process) {
      ...
      for (self in c("trans")) {
        ...
        for (cancer in cancers2process) {
          ...
          if (nrow(tab_can) > 0) {
            ...
            if (self == "trans") {
              ...
              tab4plot <- unique(tab_can[(tab_can$GENE %in% c(driver_can, SMGs[[cancer]])), c("GENE", "SUB_GENE", "SUB_MOD_RSD","Cancer", "FDR_pho_kin", "coef_pho_kin")])
        ...
        p <- p + theme(strip.background.x = element_rect(size = 1, color = "black", linetype = "solid"))
        p
        fn = paste0(subdir4, 'substrates_of_', self,'_regulated_driver_gene_', enzyme_type, '_pairs_in_TCGA_pathways_reg_nonNA',reg_nonNA, '_fdr_thres', fdr_thres, '.pdf')
        pdf(file = fn, height=8, width = 10, useDingbats = F)
        print(p)
        dev.off()
      }
    }
      ...
      for (self in c("trans")) {
        ...
        for (cancer in cancers2process) {
          ...
          if (nrow(tab_can) > 0) {
            ...
            if (self == "trans") {
              ## filter cis by showing only the SMGs
              tab4plot <- unique(tab_can[(tab_can$SUB_GENE %in% c(driver_can, SMGs[[cancer]])), c("GENE", "SUB_GENE", "SUB_MOD_RSD","Cancer", "FDR_pho_kin", "coef_pho_kin")])
            ...
            p <- p + theme(strip.background.x = element_rect(size = 1, color = "black", linetype = "solid"))
            p
            fn = paste0(subdir5, cancer, self,'_regulated_driver_gene_as_substrate_', enzyme_type, '_pairs_in_TCGA_pathways_reg_nonNA',reg_nonNA,'.pdf')
            pdf(file = fn, height=pdf_sizes[[enzyme_type]][[self]][[cancer]]$height, width = pdf_sizes[[enzyme_type]][[self]][[cancer]]$width, useDingbats = F)
            print(p)
            dev.off()
        ...
        p <- p + theme(strip.background.x = element_rect(size = 1, color = "black", linetype = "solid"))
        p
        fn = paste0(subdir4, self,'_regulated_driver_gene_as_substrate_', enzyme_type, '_pairs_in_TCGA_pathways_reg_nonNA',reg_nonNA, '_fdr_thres', fdr_thres, '.pdf')
        pdf(file = fn, height=8, width = 10, useDingbats = F)
        print(p)
        dev.off()
      }
  ...
  for (reg_nonNA in reg_nonNA2test) {
    ...
    for (fdr_thres in fdr_thres2process) {
      ...
      for (self in c("trans")) {
        ...
        pdf(file = fn, height=8, width = 6.5, useDingbats = F)
        print(p)
        dev.off()
      }
    }
