  ...
  for (enzyme_type_tmp in c("kinase")) {
    ...
    for (i in 1:2) {
      ...
      for (SELF in c("cis")) {
        ...
        tab_pairs.wratio <- fread(input = "./cptac2p/analysis_results/phospho_network/regression/figures/scatterplot_kinase_top_ratio_vs_sd/cis_kinase_for_subtype_plotting.txt", data.table = F)
        ...
        pairs2plot_df <- data.frame(pair = pairs2plot)
        pairs2plot_df <- pairs2plot_df %>%
          mutate(GENE = str_split_fixed(string = pair, pattern = ":", 3)[,1]) %>%
          mutate(SUB_MOD_RSD = str_split_fixed(string = pair, pattern = ":", 3)[,3]) %>%
          mutate(is.solo = (str_split_fixed(string = SUB_MOD_RSD, pattern = '[STY]', n = 3)[,3] == "")) %>%
          mutate(Position = str_split_fixed(string = SUB_MOD_RSD, pattern = '[STY]', n = 3)[,2])
        
        # write table for protein paint -------------------------------------------
        for (gene_tmp in unique(pairs2plot_df$GENE)) {
          tab4jude <- pairs2plot_df %>%
            filter(GENE == gene_tmp) %>%
            filter(is.solo == T) %>%
            mutate(text4proteinpaint = paste0("p.", SUB_MOD_RSD)) %>%
            mutate(judeClass = "M") %>%
            select(text4proteinpaint, Position, judeClass)
          
          write.table(x = tab4jude, file = paste0(makeOutDir(resultD = resultD), "tab4jude_", gene_tmp, "_", cancer, ".txt"), row.names = F, quote = F, sep = ";", col.names = F)
        }
        ...
        plotpheatmap(mat_value, breaks, color.palette, col_anno, row_anno, ann_colors, fn, ifelse(SELF == "cis", 10, 9), ifelse(SELF == "cis", 4, 5))
      }
    }
  }
}
