...
tab2p$coef_capped <- remove_outliers_cap(x = tab2p$coef_pro_kin, cap = x_cap)
...
p = ggplot(data = tab2p)
p = p + geom_point(mapping = aes(x=x, y= y, color = regulated), stroke = 0, alpha = 0.5, shape = 16, size = 1)
...
p
fn = paste(makeOutDir(resultD = resultD), "volcano_", SELF, "_", "cancers" , "_", enzyme_type,'.pdf',sep ="")
