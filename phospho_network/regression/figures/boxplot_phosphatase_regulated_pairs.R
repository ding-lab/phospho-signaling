...
# barplot for the top ', enzyme_type, ' with high and low kinase-substrate pairs
...
for (self in c("trans")) {
  ...
  tab4plot <- unique(sup_cans_tab_en[(sup_cans_tab_en$GENE %in% df$GENE) & !is.na(sup_cans_tab_en$GENE) & (!is.na(sup_cans_tab_en$regulated) & sup_cans_tab_en$fdr_sig) & sup_cans_tab_en$SELF == self, 
  ...
  tab4plot <- tab4plot[tab4plot$coef < 0,]
  
  # tab4plot <- tab4plot[tab4plot$SUB_GENE %in% driver_genes$Gene,]
  ...
  p <- ggplot()
  p <- p + geom_point(data=tab4plot, aes(x = GENE, y = coef, size = log10FDR, color = Cancer), 
  ...
  fn = paste0(makeOutDir(resultD = resultD),'substrates_of_', self,'_', enzyme_type, '_top_regulated_average_ratio_across_cancers.pdf')
...
#   tab4plot <- tab4plot[tab4plot$minmax_cans,]  
#   p <- ggplot()
#   p <- p + geom_point(data=tab4plot, aes(x = GENE, y = coef, fill = coef_capped, size = log10FDR, color = Cancer), 
#                       shape = 21, stroke = 1, alpha = 0.6)
#   p <- p + geom_hline(yintercept = 0, linetype = 2, color = "grey")
#   p <- p + geom_text_repel(data=tab4plot, 
#                            aes(x = GENE, y = coef, label = site, color = Cancer), size = 2.5, force = 1)
#   p <- p + ggtitle(label = paste0("phosphosites with the largest/lowest effect size correlated with each kinase across cancers (FDR<", reg_sig, ")"))
#   p <- p + scale_fill_gradientn(name= "coefficient", na.value=NA, colours=RdBu1024, limit=c(-cap,cap))
#   p <- p + scale_color_manual(values = color_cat_man)
#   p <- p + theme_bw()
#   p <- p + theme_nogrid()
#   p <- p + xlab('kinase')+ylab("effect size")
