...
for (sample_thres in seq(from = 5, to = 120, by = 5)) {
  for (enzyme_type in c("kinase")) {
    ...
    # p <-plot(fit, quantities = list(fontsize = 30), labels = list(fontsize = 30), fills = color_cancers2)
    # grid.draw(p)
    # dev.off()
...
for (self in c("cis")) {
  ...
  # p <- p + scale_y_continuous(breaks = seq(from = 10, to = 100, by = 10), labels = paste0(seq(from = 10, to = 100, by = 10), "%"))
  ...
  p <- p + ylab("number of detected/regulated pairs")
  p <- p + scale_color_manual(values = color_cancers2)
  p <- p + theme_minimal()
  p
  fn <- paste0(makeOutDir(resultD = resultD), self, "_regulated_ratio_across_thres.pdf")
