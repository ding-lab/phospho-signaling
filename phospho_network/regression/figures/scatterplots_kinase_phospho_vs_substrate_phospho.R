            ...
            if (self == "cis") {
              ...
              for (enzyme in "RAF1") {
                ...
                for (rsd in unique(tab_en$SUB_MOD_RSD)) {
                  ...
                  tab2p <- tab4plot_facet_y
                  p <- ggplot()
                  p <- p + geom_point(data = tab2p, mapping = aes(x = pro_en, y = pho_sub), alpha = 0.8, shape = 16, stroke = 0, color = "black")
                  p <- p + facet_grid(facet_y~., scales = "fixed", space = "fixed")
                  p <- p + geom_smooth(data = tab2p, mapping = aes(x = pro_en, y = pho_sub), method = "lm", se=FALSE, color="grey50", formula = y ~ x, linetype = 2)
                  p <- p + theme_bw()
                  ...
                  ggsave(filename = paste0(subdir4, cancer, "_", enzyme, "_", gene, "_", rsd, "_scatterplots.pdf"),
                         width = 3, height = pdf_sizes_per_yfacet[[as.character(length(unique(tab2p$facet_y)))]])
                }
              }
            }
            if (self == "trans") {
              ...
              for (enzyme in c("PPP1CC")) {
                ...
                for (rsd in c("S29"))) {
                  ...
                  tab2p <- tab4plot_facet_y
                  p <- ggplot()
                  p <- p + geom_point(data = tab2p, mapping = aes(x = pho_en, y = pho_sub), alpha = 0.8, shape = 16, stroke = 0, color = "black")
                  p <- p + facet_grid(facet_y~., scales = "fixed", space = "fixed")
                  p <- p + geom_smooth(data = tab2p, mapping = aes(x = pho_en, y = pho_sub), method = "lm", se=FALSE, color="grey50", formula = y ~ x, linetype = 2)
                  p <- p + theme_bw()
                  p <- p + scale_color_manual(values = brewer.pal(n = 6, name = "Set1"))
                  p <- p + xlab(paste0(enzyme, " phosphoprotein level (", cancer, ")")) + ylab(paste0(gene, " phosphosite level"))
                  p <- p + theme(strip.text.y = element_text(size = 10, face = "bold"))
                  p <- p + theme(axis.title.x = element_text(size = 12, face = "bold"), axis.title.y = element_text(size = 12, face = "bold"))
                  p
                  ggsave(filename = paste0(subdir4, cancer, "_", enzyme, "_", gene, "_", rsd, "_scatterplots.pdf"),
