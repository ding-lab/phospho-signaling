...
for (self in c("trans")) {
  ...
  p <- ggplot()
  p <- p + geom_point(data=tab4plot, aes(x = GENE, y = coef_capped, fill = coef_capped, size = log10FDR, color = Cancer), 
...
for (self in c("trans")) {
  ...
  sup_cans_tab_en_self <- sup_cans_tab_en[sup_cans_tab_en_self$SELF == self & !is.na(sup_cans_tab_en_self$pair) & (sup_cans_tab_en_self$pair %in% pair_cans_count3$Var1),]
  tab4plot <- unique(sup_cans_tab_en_self[(sup_cans_tab_en_self$uniq_BRCA | sup_cans_tab_en_self$uniq_OV | sup_cans_tab_en_self$uniq_CO), 
                                          c("GENE", "SUB_GENE", "SUB_MOD_RSD", "Cancer", "FDR_pho_kin", "coef_pho_kin")])
  colnames(tab4plot) <- c("GENE", "SUB_GENE", "SUB_MOD_RSD", "Cancer", "FDR", "coef")
  tab4plot$log10FDR <- -log10(tab4plot$FDR)
  tab4plot$site <- paste0(tab4plot$SUB_GENE, ":", tab4plot$SUB_MOD_RSD)
  ...
  tab4plot$Cancer <- factor(tab4plot$Cancer, levels = cancers_sort)
  tab4plot$GENE <- factor(tab4plot$GENE, levels = as.vector(df_count$GENE)[order(df_count$ave_ratio, decreasing = T)])
  # tab4plot <- tab4plot[tab4plot$SUB_GENE %in% driver_genes$Gene[driver_genes$Cancer.type %in% c(cancers_sort, "PANCAN")],]
