...
for (cancer in cancers2process) {
  ...
  genes2process <- unique(esscore_tab_outlier_drug$GENE[esscore_tab_outlier_drug$cancer == cancer & esscore_tab_outlier_drug$is.direct])
...
for (cancer in cancers2process) {
  ...
  # if (!file.exists(fn)) {
    ...
    for (geneA in names(pairs2plot[[cancer]])) {
      ...
      if (nrow(mut_mat_tmp) > 0){
        ...
        for (gene in unique(mut_mat.m$Gene[mut_mat.m$variant_class_sim != "wild_type"])) {
          ...
          df_outlier <- rbind(df_outlier, mut_mat2merge)
        }
      } else {
        print("no mutation!")
      }
      ...
      if (nrow(cna_tab_tmp) > 0) {
        ...
        for (gene in unique(cna_tab.m$gene[cna_tab.m$CNA != "neutral"])) {
          ...
          df_outlier <- rbind(df_outlier, cna_mat2merge)
        }
      } else {
        print("no CNA!")
      }
      ...
      if (!is.null(rna_tab_tmp)) {
        if (nrow(rna_tab_tmp) > 0) {
          ...
          df_outlier <- rbind(df_outlier, rna_outlier2merge)
        }
      }
      ...
      if (!is.null(pro_outlier_tmp)) {
        if (nrow(pro_outlier_tmp) > 0) {
          ...
          pro_outlier2merge <- pro_outlier.m %>%
            ...
            select(partID, expression_type, row_id, color_value)
          df_outlier <- rbind(df_outlier, pro_outlier2merge)
        }
      }
      ...
      if (!is.null(phog_outlier_tmp)) {
        if (nrow(phog_outlier_tmp) > 0) {
          ...
          df_outlier <- rbind(df_outlier, phog_outlier2merge)
        }
      }
          ...
          if (nrow(esscore_tab_outlier.m) > 0) {
            for (pair_tmp in unique(esscore_tab_outlier.m$pair[!is.na(esscore_tab_outlier.m$value) & esscore_tab_outlier.m$value == T])) {
              ...
              esscore_tab_outlier2merge <- esscore_tab_outlier2merge %>%
                ...
                select(partID, expression_type, row_id, color_value)
              df_outlier <- rbind(df_outlier, esscore_tab_outlier2merge)
            }
          }
        }
      }
      ...
      # plot upset plot ---------------------------------------------------------
      data_levels2process <- c("Mutation", "CNA", "RNA", "PRO", "PHO", "KS")
      tab2upset <- data.frame(partID = partIDs)
      for (expression_type_tmp in data_levels2process) {
        tab2upset[, expression_type_tmp] <- as.numeric(tab2upset$partID %in% df_outlier$partID[(df_outlier$expression_type == expression_type_tmp & df_outlier$color_value > 0 & !is.na(df_outlier$color_value))])
      }
      
      sub_fn = paste(subdir1, geneA, "_", cancer , '_outlier.pdf',sep ="")
      pdf(sub_fn, height = 3, width = 4, onefile = F)
      upset(tab2upset, sets = intersect(c("Mutation", "CNA", "RNA", "PRO", "PHO", "KS"), unique(df_outlier$expression_type[df_outlier$color_value > 0 & !is.na(df_outlier$color_value)])), sets.bar.color = "#56B4E9", keep.order = T,
            empty.intersections = NULL)
      dev.off()
      
      tab2upset_sup <- rbind(tab2upset_sup, tab2upset)
      
      # make the matrix for the heatmap body ------------------------------------
      # df_outlier <- data.frame(table(tab2upset[, data_levels2process]))
      # df_outlier$GENE <- geneA
      # df_value_sup <- rbind(df_value_sup, df_outlier)
      
      # add the gaps between columns --------------------------------------------
    ...
    # df_value_sup <- df_value_sup %>%
    #   filter(Freq > 0) %>%
    #   mutate(row_id = paste(Mutation, CNA, RNA, PRO, PHO, KS, sep = "_")) %>%
    #   filter(row_id != "0_0_0_0_0_0") %>%
    #   mutate(Ratio = Freq/length(partIDs)) %>%
    #   arrange(-Freq)
    # 
    # df_value_sup %>% nrow()
    # 
    # mat_value <- dcast(data = df_value_sup, GENE ~ row_id, value.var = "Ratio")
    # mat_value <- mat_value[order(mat_value$GENE),]
    # rownames(mat_value) <- mat_value$GENE
    # mat_value <- as.matrix(mat_value[,-1])
    # 
    # mat_label = as.character(round(x = mat_value*100, digits = 1))
    # mat_label[is.na(mat_label)] <- ""
    # mat_label <- matrix(data = as.matrix(mat_label), nrow = nrow(mat_value), ncol = ncol(mat_value))
    # 
    # # add row annotation ------------------------------------------------------
    # col_anno_df <- as.data.frame(str_split_fixed(string = colnames(mat_value), pattern = "_", n = length(data_levels2process)))
    # colnames(col_anno_df) <- data_levels2process
    # rownames(col_anno_df) <- colnames(mat_value)
    # col_anno_df <- col_anno_df[, rev(data_levels2process)]
    # 
    # for (data_level_tmp in data_levels2process) {
    #   ann_colors[[data_level_tmp]] <- c("0" = "white", "1" = "black")
    # }
    # # plotting heatmap --------------------------------------------------------
    # breaks <- seq(0, 0.3, 0.05)
    # my_heatmap <- pheatmap(mat_value, color = col_paletteR(length(breaks)), 
    #                        breaks = breaks, 
    #                        annotation_col = col_anno_df, display_numbers = mat_label, 
    #                        # color = c("#d9d9d9", rainbow(length(unique(df_outlier$color_value[!is.na(df_outlier$color_value) & df_outlier$color_value > 0])))),
    #                        # gaps_row = gaps_row_vector,
    #                        na_col = "grey50", cellwidth = 18, cellheight = 15,
    #                        cluster_rows=F, cluster_cols=F,
    #                        number_color = "black", fontsize_number = 8, fontsize_row = 10,
    #                        annotation_legend = F,
...
sub_fn = paste(makeOutDir(resultD = resultD) , 'all_cancers_outlier.pdf',sep ="")
pdf(sub_fn, height = 4, width = 6, onefile = F, useDingbats = F)
upset(tab2upset_sup, 
      sets = rev(c("Mutation", "CNA", "RNA", "PRO", "PHO", "KS")), 
      sets.bar.color = "#56B4E9", keep.order = T, order.by = "degree",
      empty.intersections = NULL)
dev.off()
