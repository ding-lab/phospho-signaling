...
plotpheatmap <- function(mat_value, color.palette, col_anno, ann_colors, width, height, mat_label) {
  result <- tryCatch({
    if (is.null(ann_colors)) {
      my_heatmap <- pheatmap(mat_value, 
                             color = color.palette,
                             scale = "row",
                             na_col = "white", annotation_col = col_anno,
...
# bussiness ------------------------------------------------------------------
# for (cancer in c("UCEC", "BRCA", "OV", "CO", "CCRCC")) {
  ...
  if (!file.exists(fn)) {
    ...
    for (geneA in names(pairs2plot[[cancer]])) {
      ...
      row_order <- c(paste0(geneA, "_", "PRO"), site_ids[grepl(pattern = geneA, x = site_ids)], 
                     paste0(geneA, "_", "collapsed_PHO"))
      mat_value <- as.matrix(mat_value)
      partIDs2filter <- intersect(row_order, rownames(mat_value))
      
      if (length(partIDs2filter) > 1) {
        mat_value <- mat_value[partIDs2filter,]
      } else {
        mat_value <- t(as.matrix(mat_value[partIDs2filter,]))
        rownames(mat_value) <- intersect(row_order, rownames(mat_value))
      }
      ...
      mat_value <- mat_value[rownames(mat_value) != "",]
      mat_values_sup <- rbind(mat_values_sup, mat_value)
      mat_labels_sup <- rbind(mat_labels_sup, mat_label)
      col_anno_sup <- merge(col_anno_sup, col_anno, by = intersect(colnames(col_anno_sup), colnames(col_anno)), all.x = T)
      
    }
    mat_labels_sup <- mat_labels_sup[rownames(mat_values_sup), colnames(mat_values_sup)]
    mat_labels_sup[!is.na(mat_labels_sup)] <- "x"
    mat_labels_sup[is.na(mat_labels_sup)] <- ""
    rownames(col_anno_sup) <- col_anno_sup$partID
    col_anno_sup$partID <- NULL
    stop()
    if (!("subtype" %in% col_anno_sup)) {
      plotpheatmap(mat_value = mat_values_sup, color.palette = color.palette, ann_colors = ann_colors, width = 20, height = 0.1*nrow(mat_values_sup) + 3, mat_label = mat_labels_sup, col_anno = NULL)
      
    } else {
      plotpheatmap(mat_values_sup, color.palette, col_anno_sup, ann_colors, 20, 0.1*nrow(mat_values_sup) + 3, mat_labels_sup)
