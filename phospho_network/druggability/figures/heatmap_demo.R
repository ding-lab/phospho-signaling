...
# input varibles about MET -----------------------------------------------
# mut_genes <- c("MET")
# cna_genes <- c("MET")
# rna_genes <- c("VEGFA", "MET", "KDR", "FLT1")
# pro_genes <- c("VEGFA", "MET", "KDR", "FLT1")
# pho_genes <- c("MET")
# rsds <- c("Y1234")
# phog_genes <- c("")
# pairs2plot <- c("MET:MET:Y1234")
# cancers2process <- c("CCRCC", "CO", "OV", "UCEC")

# input varibles about AKT1 -----------------------------------------------
# mut_genes <- c("AKT1", "PTEN", "PIK3CA", "EGFR")
# cna_genes <- c("AKT1", "PTEN", "PIK3CA", "EGFR")
# rna_genes <- c("")
# pro_genes <- c("")
# pho_genes <- c("LMNA", "VIM")
# rsds <- c("S301", "S226")
# phog_genes <- c("AKT1")
# pairs2plot <- c("AKT1:LMNA:S226")
# cancers2process <- c("CCRCC")

# input varibles about MTOR -----------------------------------------------
mut_genes <- c("AKT1", "AKT2", "AKT3", "PTEN", "PIK3CA", "EGFR")
cna_genes <- c("AKT1", "PTEN", "PIK3CA", "EGFR")
rna_genes <- c("")
pro_genes <- c("")
pho_genes <- c("MTOR", "AKT1S1", "EIF4EBP1")
rsds <- c("S2450", "S183", "S65")
phog_genes <- c("MTOR")
pairs2plot <- c("AKT1:LMNA:S301")
cancers2process <- c("CCRCC")
...
for (pair in pairs2plot) {
  ...
  for (cancer in cancers2process) {
    ...
    if (!file.exists(fn)) {
      ...
      if (nrow(pho_tab) > 0) {
        pho_tab <- pho_tab %>%
          mutate(row_id = paste0(Gene, "_", Phosphosite)) %>%
      ...
      plotpheatmap(mat_value, color.palette, col_anno, ann_colors, 20, 6)
