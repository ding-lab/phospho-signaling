...
# input dependencies ------------------------------------------------------
result_df <- fread(input = "./analysis_results/phospho_network/genoalt/tables/test_ccRCC_SMG_mut_impact_global_protein_RNA/20210331.v1/ccRCC_not_silent_mut_impact_PRO.tsv", data.table = F)

# make plot data frame ----------------------------------------------------
plot_data_df <- result_df %>%
  mutate(is_significant = (fdr_by_gene <= sig_fdr_thres))
  
## filter for affected genes to show
genes_affected_plot <- plot_data_df$SUB_GENE[plot_data_df$is_significant]
plot_data_df <- plot_data_df %>%
  filter(SUB_GENE %in% genes_affected_plot) %>%
  mutate(log10_FDR = (-log10(fdr_by_gene)))
...
cap <- 0.5
plot_data_df <- plot_data_df %>%
  dplyr::mutate(meddiff_capped = ifelse(meddiff > cap, cap, ifelse(meddiff < (-cap), -cap, meddiff))) %>%
  dplyr::arrange(GENE)
plot_data_df$SUB_GENE <- factor(x = plot_data_df$SUB_GENE, levels = unique(plot_data_df$SUB_GENE))
# make colors -------------------------------------------------------------
...
# save output -------------------------------------------------------------
file2write <- paste0(dir_out, "Mut_Impact_PRO.png")
png(file2write, width = 500, height = 500, res = 150)
print(p)
dev.off()
file2write <- paste0(dir_out, "Mut_Impact_PRO.pdf")
