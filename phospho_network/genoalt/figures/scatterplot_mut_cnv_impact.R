...
for (genoalt_type in c("amplification", "deletion")) {
  ...
  for (cancer in cancers_sort) {
    tab_mut <- mut_cna_sig_cans[mut_cna_sig_cans$cancer == cancer & mut_cna_sig_cans$genoalt_type == genoalt_type,]
    ...
    for (i in 1:nrow(tab_mut)) {
      ...
      if (cancer == "BRCA") {
        sup_tab$subtype <- sampID2pam50(sampleID_vector = as.vector(sup_tab$variable), subtype_map = loadPAM50Map())
      }
      if (cancer == "CO") {
        sup_tab$subtype <- "other"
        sup_tab$subtype[sup_tab$partID %in% partIDs_msih] <- "MSI-High"
