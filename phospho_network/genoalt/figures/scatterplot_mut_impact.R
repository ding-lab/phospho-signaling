...
# for (cancer in "BRCA") {
  ...
  for (self in c("cis")) {
    ...
    # tab_mut <- mut_cnv_tab[mut_cnv_tab$SUB_GENE %in% c("CTNNB1", "APC") & mut_cnv_tab$GENE %in% c("APC"),]
    tab_mut <- mut_cnv_tab[mut_cnv_tab$SUB_GENE %in% c("TP53") & mut_cnv_tab$GENE %in% c("TP53"),]
    ...
    # tab_mut <- pho_head[pho_head$SUBSTRATE == "PDK1",]
    if (nrow(tab_mut) == 0) {
      ...
      for (i in 1:nrow(tab_mut)) {
        ...
        if (enzyme %in% c("APC", "CTNNB1", "TP53")) {
        ...
        p = p + labs(y=paste0(substrate, " ", rsd, " phosphorylation abundance(log2 ratio) in", cancer),
                     x = paste0(enzyme, " phosphorylation(log2 ratio) in ", cancer))
        p = p + theme_nogrid()
