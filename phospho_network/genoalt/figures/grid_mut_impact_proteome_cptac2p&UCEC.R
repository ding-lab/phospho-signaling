...
# Input -------------------------------------------------------------------
mut_cnv_cans_sup <- fread(input = paste0(ppnD, "genoalt/tables/test_mut_impact_proteome/mut_impact_proteome_cptac2p_tab.txt"), data.table = F, sep = "\t")


# set variables -----------------------------------------------------------
show_p_thres <- 0.2
sig_p_thres <- 0.05
cancers2process <- unique(mut_cnv_cans_sup$cancer)
driver_genes2show <- unique(c(as.vector(driver_genes$Gene[driver_genes$Cancer %in% cancers2process]), unlist(SMGs)))
...
for (variant_class in c("missense", "not_silent", "truncation")) {
  affected_exp_type <- "PRO"
  do.smg <- FALSE
  if (affected_exp_type == "PRO") {
    for (genoalt_type in c("mut")) {
      mut_cnv_cans <- mut_cnv_cans_sup[mut_cnv_cans_sup$num >= num_genoalt_thres & mut_cnv_cans_sup$variant_class  ==  variant_class,]
      ...
      if (do.smg) {
        enzymes2show <- intersect(smg_cancer, mut_cnv_cans$GENE[mut_cnv_cans$p < sig_p_thres])
      } else {
        enzymes2show <- unique(mut_cnv_cans$GENE[mut_cnv_cans$p < sig_p_thres & mut_cnv_cans$p > 0 & mut_cnv_cans$genoalt_type == genoalt_type & (mut_cnv_cans$GENE %in% driver_genes2show | mut_cnv_cans$driver_gene_type.en != "")])
      ...
      p
      fn = paste(makeOutDir(resultD = resultD), genoalt_type, "_impact_", affected_exp_type, "_num_genoalt_thres_", num_genoalt_thres, "_do.smg_", do.smg, '.pdf',sep = "")
      if (do.smg) {
        ggsave(filename = fn, width = 8, height = 4)
      } else {
  ...
  if (affected_exp_type == "PHO") {
    for (genoalt_type in c("mut")) {
      ...
      df <- mut_cnv_cans[mut_cnv_cans$genoalt_type == genoalt_type,]
      df <- df[(df$GENE %in% enzymes2show) & (df$SUB_GENE %in% subtrates2show),]
      enzymes2show <- intersect(enzymes2show, df$GENE[df$p < sig_p_thres])
      df <- df[(df$GENE %in% enzymes2show) & (df$SUB_GENE %in% subtrates2show),]
      df$sig <- (df$p < sig_p_thres)
      ...
      p <- p + geom_point(data = df, mapping = aes(x = SUB_GENE, y = GENE, fill = meddiff_capped, size = log10_Pvalue,
                                                   colour = ifelse(sig == TRUE, paste0('P-value<', sig_p_thres), paste0('P-value>', sig_p_thres))), shape = 21, alpha = 0.6)
      p <- p + scale_color_manual(values = sig_colors)
      p = p + scale_fill_gradientn(name= "phosphoprotein change", na.value=NA, colours=RdBu1024, limit=c(-cap,cap))
      p <- p + facet_grid(driver_gene_type.en ~ SUB_GENE.path, drop=T, space = "free",scales = "free")#, space = "free", scales = "free")
      p <- p + xlab("Associated Phosphorotein") + ylab("Mutated Gene")
      p <- p + guides(colour = guide_legend(title = "P-value"))
      p <- p + theme_bw()
      p <- p + theme(panel.spacing.x = unit(0, "lines"), panel.spacing.y = unit(0, "lines"))
      p <- p + theme(axis.text.y = element_text(size = 10, face = "bold"))
