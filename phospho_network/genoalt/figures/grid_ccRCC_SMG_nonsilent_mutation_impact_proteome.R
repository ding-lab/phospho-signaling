...
# input dependencies ------------------------------------------------------
result_df <- fread(input = "./analysis_results/phospho_network/genoalt/tables/test_ccRCC_SMG_mut_impact_global_protein_RNA/20210329.v1/ccRCC_not_silent_mut_impact_PRO.tsv", data.table = F)
...
png(file2write, width = 300, height = 400, res = 150)
print(p)
dev.off()
file2write <- paste0(dir_out, "PDA_Mut_Impact_PRO.pdf")
pdf(file2write, width = 2, height = 3, useDingbats = F)
print(p)
dev.off()
