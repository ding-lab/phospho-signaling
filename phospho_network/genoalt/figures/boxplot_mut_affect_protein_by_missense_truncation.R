...
exp_data <- pro_df[, colnames_pro[grepl(pattern = "_T", x = colnames_pro) & !grepl(pattern = "\\.1", x = colnames_pro)]]
exp_sampleids <- gsub(x = colnames(exp_data), pattern = "_T", replacement = "")
colnames(exp_data) <- exp_sampleids
