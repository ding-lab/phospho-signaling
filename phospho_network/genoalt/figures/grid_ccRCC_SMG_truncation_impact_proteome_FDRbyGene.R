...
# input dependencies ------------------------------------------------------
result_df <- fread(input = "./analysis_results/phospho_network/genoalt/tables/test_ccRCC_SMG_mut_impact_global_protein_RNA/20210331.v1/ccRCC_truncation_mut_impact_PRO.tsv", data.table = F)
...
plot_data_df <- plot_data_df %>%
  mutate(meddiff_capped = ifelse(meddiff > cap, cap, ifelse(meddiff < (-cap), -cap, meddiff))) %>%
  arrange(Gene)
