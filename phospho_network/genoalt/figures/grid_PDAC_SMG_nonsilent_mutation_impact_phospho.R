...
# input dependencies ------------------------------------------------------
result_df <- fread(input = "./analysis_results/phospho_network/genoalt/tables/test_PDA_SMG_mut_impact_global/20201130.v1/PDAC_not_silent_mut_impact_PHO.tsv", data.table = F)
...
genes2pathways_filtered_df <- genes2pathways_df %>%
  arrange(SUB_GENE) %>%
  filter(SUB_GENE == "MAP2K1" & SUB_GENE.path == "RTK RAS" | SUB_GENE != "MAP2K1") %>%
  filter(SUB_GENE == "PRKCA" & SUB_GENE.path == "RTK RAS" | SUB_GENE != "PRKCA") %>%
  filter(SUB_GENE == "EGFR" & SUB_GENE.path == "RTK RAS" | SUB_GENE != "EGFR") %>%
  filter(SUB_GENE == "CCNE2" & SUB_GENE.path == "Cell Cycle" | SUB_GENE != "CCNE2") %>%
  filter(SUB_GENE == "EP300" & SUB_GENE.path == "TGF beta" | SUB_GENE != "EP300")
## map pathway
...
plot_data_df$SUB_GENE.path <- factor(x = subgene_path_vec, levels = c("RTK RAS", "Cell Cycle", "Mismatch repair", "PI3K", "Other"))
...
# plot --------------------------------------------------------------------
p <- ggplot()
p <- p + geom_point(data = plot_data_df, mapping = aes(x = SUB_GENE, y = GENE, fill = meddiff_capped, size = log10_FDR, shape = SELF), alpha = 0.6, color = "black")
p <- p  + scale_shape_manual(values = c("cis" = 22, "trans" = 21))
p <- p + scale_color_manual(values = sig_colors)
p = p + scale_fill_gradientn(name= "phosphorylation\nchange", na.value=NA, colours=colors_exp, limit=c(-cap,cap))
...
# save output -------------------------------------------------------------
file2write <- paste0(dir_out, "PDA_Mut_Impact_PHO.png")
png(file2write, width = 1000, height = 600, res = 150)
print(p)
dev.off()
file2write <- paste0(dir_out, "PDA_Mut_Impact_PHO.pdf")
