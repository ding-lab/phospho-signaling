...
enzyme_sub_cis <- data.frame(GENE = "TP53", SUB_GENE = "TP53")
...
for (cancer in c("UCEC")) {
  ...
  for (enzyme in unique(enzyme_sub_sites$GENE)) {
    ...
    if (nrow(mut_mat_en) > 0){
      ...
      mut_missense_partIDs <- partIDs_overlap[which(grepl(pattern = "Missense", x = mut_mat_en[,partIDs_overlap]))]
      mut_truncation_partIDs <- mut_partIDs[!(mut_partIDs %in% mut_missense_partIDs)]
    } else {
      mut_partIDs <- NULL 
    }
...
tab2p <- data.frame(value = as.vector(t(pho_sub[, c(mut_missense_partIDs, mut_truncation_partIDs, control_partIDs)])),
                    group = c(rep("Missense", length(mut_missense_partIDs)), rep("Truncation", length(mut_truncation_partIDs)), rep("Control", length(control_partIDs))))
library(ggplot2)
p <- ggplot()
p <- p + geom_boxplot(data = tab2p, mapping = aes(x = group, y = value))
p
