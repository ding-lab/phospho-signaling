...
for (cancer in c("BRCA", "OV", "CO", "UCEC")) {
  ...
  complex_pair_tab_sites <- merge(complex_pair_tab_genoalt_thresholded, pro_head, by.x = c("geneB"), by.y = c("SUBSTRATE"), all.y = T)
  ...
  for (enzyme in unique(complex_pair_tab_sites$geneA)) {
    ...
    for (substrate in unique(complex_pair_tab_sites$geneB[complex_pair_tab_sites$geneA == enzyme])) {
      ...
      i <- (complex_pair_tab_sites$geneA == enzyme & complex_pair_tab_sites$geneB == substrate)
