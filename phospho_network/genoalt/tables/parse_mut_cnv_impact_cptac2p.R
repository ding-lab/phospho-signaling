...
mut_cnv_pho <- fread(input = paste0(ppnD, "genoalt/tables/test_mut_cna_impact_cptac2p/mut_cnv_sig_cans.txt"), data.table = F)

# input and parse mutaional impact on protein within enzyme-substrate pair (BRCA+OV+CO+UCEC) ---------------------------
mut_cnv_pro <- fread(input = paste0(ppnD, "genoalt/tables/test_mut_enzyme_substrate_pro/mut_cnv_sig_cans.txt"), data.table = F)


# merge phosphoprotein and protein affected by mutation/CNA-------------------------------------------------------------------
col2merge <- intersect(colnames(mut_cnv_pho), colnames(mut_cnv_pro))
