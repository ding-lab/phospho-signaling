...
cancers2process <- c("BRCA", "CO", "OV", "UCEC", "CCRCC")
  ...
  for (variant_class in variant_classes2process) {
    for (cancer in cancers2process) {
      ...
      if (!file.exists(fn)) {
        ...
        mut_mat <- generate_somatic_mutation_matrix(pair_tab = SMGs[[cancer]], maf = maf)
        ...
        regulated_phosphosites <- regression %>%
          filter(regulated == T & Cancer == cancer) %>%
        ...
        if (affected_exp_type %in% c("PRO", "RNA")) {
          pair_tab_sites_tail <- data.frame(SUB_GENE = regulated_substrates[rep(1:nrow(regulated_substrates), length(genes_num_genoalt_thresholded)),])
          pair_tab_sites_head <- data.frame(GENE = rep(genes_num_genoalt_thresholded, nrow(regulated_substrates) + vector(mode = "numeric", length = length(genes_num_genoalt_thresholded))))
          pair_tab_sites <- cbind(pair_tab_sites_head, pair_tab_sites_tail)
          pair_tab_sites$SUB_MOD_RSD <- affected_exp_type
        } else {
          pair_tab_sites_tail <- regulated_phosphosites[rep(1:nrow(regulated_phosphosites), length(genes_num_genoalt_thresholded)),]
          pair_tab_sites_head <- data.frame(GENE = rep(genes_num_genoalt_thresholded, nrow(regulated_phosphosites) + vector(mode = "numeric", length = length(genes_num_genoalt_thresholded))))
          pair_tab_sites <- cbind(pair_tab_sites_head, pair_tab_sites_tail)
        }
        pair_tab_sites <- pair_tab_sites[!is.na(pair_tab_sites$GENE) & !is.na(pair_tab_sites$SUB_MOD_RSD),]
        pair_tab_sites <- unique(pair_tab_sites)
...
mut_cnv_cans$fdr <- FDR_by_id_columns(p_vector = mut_cnv_cans$p, id_columns = c("SELF", "cancer", "variant_class", "affected_exp_type"), df = mut_cnv_cans)
mut_cnv_cans$fdr_by_gene <- FDR_by_id_columns(p_vector = mut_cnv_cans$p, id_columns = c("GENE", "SELF", "cancer", "variant_class", "affected_exp_type"), df = mut_cnv_cans)
