...
for (cancer in c("BRCA", "OV", "CO")) {
  mut_cnv_can <- fread(input = paste0("./cptac2p/analysis_results/phospho_network/genoalt/tables/test_mut_cna_impact_cis_pro_cptac2p/", cancer, "_mut_cnv_tab_formatted.txt"), data.table = F)
  mut_cnv_cans <- rbind(mut_cnv_cans, mut_cnv_can)
}
