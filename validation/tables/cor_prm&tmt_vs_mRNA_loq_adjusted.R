...
## adjust PRM values to eliminate those under LOQ (lowest LOQ if multiple peptides)
for (i in 1:nrow(prm_PPI_labeled)) {
  tmp <- prm_PPI_labeled[i, -1]
  gene <- prm_PPI_labeled[i, "Gene_Name"]
  loqs <- prm_raw.f$CPTAC.LLOQ[prm_raw.f$Gene_Name == gene]
  loqs <- loqs[!is.na(loqs)]
  if (length(loqs) > 0) {
    loq <- min(loqs, na.rm = T)
  } else {
    loq <- 0
  }
  tmp[tmp <= loq] <- NA
  prm_PPI_labeled[i, -1] <- tmp
}
...
for (method in c("spearman", "pearson")) {
  cor_stats <- lapply(1:length(prm_mrna_genes_int), FUN = function(n, m1, m2, gs, ps, mt) try(cor.test(x = as.numeric(as.vector(m1[m1$Gene_Name == gs[n], ps])), 
...
# tmt~mRNA ----------------------------------------------------------------
## get intersecting genes and patient IDs
tmt_mrna_genes_int <- intersect(intersect(tmt_PPI_labeled.t$gene, fpkm$gene), prm_mrna_genes_int)
tmt_mrna_partIDs_int <- intersect(colnames(tmt_PPI_labeled.t), colnames(fpkm))
tmt_mrna_partIDs_int <- tmt_mrna_partIDs_int[!grepl(pattern = "gene", x = tmt_mrna_partIDs_int, ignore.case = T)]

for (method in c("spearman", "pearson")) {
  cor_stats <- lapply(1:length(tmt_mrna_genes_int), FUN = function(n, m1, m2, gs, ps, mt) try(cor.test(x = as.numeric(as.vector(m1[m1$gene == gs[n], ps])), 
                                                                                                       y = as.numeric(as.vector(m2[m2$gene == gs[n], ps])), 
                                                                                                       method = mt), silent = T),
                      m1 = tmt_PPI_labeled.t, m2 = fpkm.log2, gs = tmt_mrna_genes_int, ps = tmt_mrna_partIDs_int, mt = method)
  cor_df <- data.frame(Gene = tmt_mrna_genes_int,
                       pvalue = sapply(1:length(tmt_mrna_genes_int), FUN = function(n, l) {
                         if ("p.value" %in% names(l[[n]])) {
                           return(l[[n]]$p.value)
                         } else {
                           return(NA)
                       ...
                       rho = sapply(1:length(tmt_mrna_genes_int), FUN = function(n, l) {
                         if ("estimate" %in% names(l[[n]])) {
                           return(l[[n]]$estimate[[1]])
                         } else {
                           return(NA)
                       ...
                       num_samples = sapply(1:length(tmt_mrna_genes_int), FUN = function(n, m1, m2, gs, ps) length(which(!is.na(m1[m1$gene == gs[n], ps]) & !is.na(m2[m2$gene == gs[n], ps]))),
                                            m1 = tmt_PPI_labeled.t, m2 = fpkm.log2, gs = tmt_mrna_genes_int, ps = tmt_mrna_partIDs_int))
  
  cor_df$fdr <- p.adjust(cor_df$pvalue,method = "fdr")
  cor_df$log_fdr <- -log10(cor_df$fdr)
  ## add in the group name for kinases
  cor_df <- merge(cor_df, kinase_family_per_synonym[!duplicated(kinase_family_per_synonym$synonym), c("synonym", "GroupName", "ProteinNames")], by.x = c("Gene"), by.y = c("synonym"), all.x = T)
  cor_df$is_kinase <- (grepl(pattern = "kinase", x = as.vector(cor_df$ProteinNames)) | !is.na(cor_df$ProteinNames))
  cor_df.k <- cor_df[cor_df$is_kinase,]; cor_df.k$panel <- "Kinome_panel"
  ## add in the group name for metabolic enzymes
  cor_df.m <- cor_df[!cor_df$is_kinase,]; cor_df.m$GroupName <- NULL; cor_df.m$ProteinNames <- NULL
