...
for (method in c("spearman", "pearson")) {
  cor_df <- fread(input = paste0("./cptac2p/cptac_shared/analysis_results/validation/tables/cor_prm&tmt_vs_mRNA_loq_adjusted/", cancer, "_", method, "_correlation_gene_level_prm_mRNA.txt"))
...
for (method in c("spearman", "pearson")) {
  cor_df <- fread(input = paste0("./cptac2p/cptac_shared/analysis_results/validation/tables/cor_prm&tmt_vs_mRNA_loq_adjusted/", cancer, "_", method, "_correlation_gene_level_tmt_mRNA_limittoprmgenes.txt"))
  cor_df <- data.frame(cor_df)
  for (panel in unique(cor_df$panel)) {
    cor_df_panel <- cor_df[cor_df$panel == panel,]
    for (sig in c(0.05, 0.1)) {
