...
for (method in c("spearman")) {
  sig <- 0.1
  cor_prm <- fread(input = paste0("./cptac2p/cptac_shared/analysis_results/validation/tables/cor_prm&tmt_vs_mRNA_loq_adjusted/", cancer, "_", method, "_correlation_gene_level_prm_mRNA.txt"))
  cor_prm <- data.frame(cor_prm)
  cor_tmt <- fread(input = paste0("./cptac2p/cptac_shared/analysis_results/validation/tables/cor_prm&tmt_vs_mRNA_loq_adjusted/", cancer, "_", method, "_correlation_gene_level_tmt_mRNA_limittoprmgenes.txt"))
  ...
  for (panel in unique(cor_df$panel)) {
    ...
    p = p + theme(title = element_text(size = 8),
                  ...
                  legend.title = element_text(size = 5), legend.justification = c(0, 0), legend.position = c(0, 0))
    p
