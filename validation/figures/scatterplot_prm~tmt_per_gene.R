...
## scatter plot for comparing prospective BRCA PRM and TMT protein-level abundance
...
prm_tmt_genes_int <- intersect(prm_PPI_labeled$Gene_Name, tmt_PPI_labeled.t$gene)
prm_tmt_partIDs_int <- intersect(colnames(prm_PPI_labeled), colnames(tmt_PPI_labeled.t))
...
for (method in c("spearman")) {
  cor_df <- fread(input = paste0("./cptac2p/cptac_shared/analysis_results/validation/tables/cor_prm_ms_match_specimen/", cancer, "_", method, "_correlation_prm_ms_match_parent_specimen.txt"))
  for (gene in cor_df$Gene) {
    df <- data.frame(prm = as.numeric(as.vector(prm_PPI_labeled[prm_PPI_labeled$Gene == gene, prm_tmt_partIDs_int])),
                     tmt = as.numeric(as.vector(tmt_PPI_labeled.t[tmt_PPI_labeled.t$gene == gene, prm_tmt_partIDs_int])))
    p = ggplot(df, aes(x=prm, y=tmt))
    p = p + geom_point(alpha=0.5, stroke = 0)
    p = p + labs(x = paste0("PRM protein abundance"), 
                 y = paste0("global protein abundance"))
    if (!is.na(cor_df$rho[cor_df$Gene == gene])) {
      p = p + annotate("text", x = quantile(df$prm, probs = 0.985, na.rm = T), y = quantile(df$tmt, probs = 0.999, na.rm = T), label = paste0("spearman's rho = ", signif(unique(cor_df$rho[cor_df$Gene == gene]), digits = 2)))
      p = p + annotate("text", x = quantile(df$prm, probs = 0.985, na.rm = T), y = quantile(df$tmt, probs = 0.999, na.rm = T)-(range(df$tmt, na.rm = T)[2]-range(df$tmt, na.rm = T)[1])/30, label = paste0("FDR = ", signif(unique(cor_df$fdr[cor_df$Gene == gene]), digits = 2)))
    } else {
      p = p + annotate("text", x = quantile(df$prm, probs = 0.985, na.rm = T), y = quantile(df$tmt, probs = 0.999, na.rm = T), label = paste0("spearman's rho = ", "NA"))
      p = p + annotate("text", x = quantile(df$prm, probs = 0.985, na.rm = T), y = quantile(df$tmt, probs = 0.995, na.rm = T), label = paste0("FDR = ", "NA"))
    }
    p = p + theme_nogrid()
    ...
    p
    resultDnow <- makeOutDir()
    fn = paste0(subdir, cancer, "_", gene, "_prm~tmt.pdf")
...
for (gene in prm_tmt_mrna_genes_int) {
  ...
  p = p + geom_point(alpha=0.5, stroke = 0)
  p <- p + facet_grid(technology~., scales = "free_y", space = "free", heights = c(1,1))
