...
## volcano plot for correlation of prospective BRCA PRM peptide-level abundance with MS/MS protein abundance from CDAP
...
for (method in c("pearson", "spearman")) {
  ...
  for (panel in unique(cor_df$panel)) {
    ...
    for (sig in c(0.05, 0.1)) {
      sink(file = paste0(resultDnow, cancer, "_", panel, "_", method, "_correlation_gene_level_prm_tmt", "_fdr",sig,".txt"))
      cat(paste0(length(which(!is.na(cor_df_panel$rho))), " proteins in ", panel, " were tested for correlation btw prm and global protein abundance\n"))
      cat(paste0(length(which(cor_df_panel$rho > 0)), " proteins in ", panel, " has positive correlation btw prm and global protein abundance\n"))
      cat(paste0(length(which(cor_df_panel$rho > 0 & cor_df_panel$fdr <= sig)), " proteins in ", panel, " has significant positive correlation btw prm and global protein abundance\n"))
      sink()
      closeAllConnections()
      ...
      p = p + ggtitle(label = paste0(panel, "(", length(which(!is.na(cor_df_panel$rho))), " proteins)" ), 
                      subtitle = paste0(method, " correlation of PRM and TMT detected abundance(N=", length(parent_sampIDs_int), ")"))
      p <- p + facet_grid(.~panel, scales = "free", space = "free")
      p = p + theme_nogrid()
      ...
      fn = paste0(resultDnow, cancer, "_", panel, "_", method,'_correlation_prm_ms_match_parent_specimen_for_kinases_in_', cancer, "_fdr",sig,".pdf")
