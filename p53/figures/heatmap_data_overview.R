...
# for (cancer in c("BRCA", "OV", "CO", "UCEC", "CCRCC")) {
  ...
  col_anno <- sup_tab[sup_tab$Phosphosite == "PRO", c("partID", "variant_class_sim", "is.deleted", "exp_value")]
  col_anno %>% head()
  ...
  colnames(col_anno) <- c("mutation", "deletion")
  ...
  if (cancer == "LIHC") {
    ...
    save_pheatmap_pdf(x = my_heatmap, 
                      filename = paste0(makeOutDir(resultD = resultD), "TP53_", cancer, "_overview.pdf"), 
                      width = 12, height = 1)
  } else {
    my_heatmap <- pheatmap(mat_value, 
                           color = color.palette,
