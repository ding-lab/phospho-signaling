...
for (cancer_tmp in c("BRCA", "OV", "CO", "UCEC", "LIHC")) {
  ...
  for (expression_type_tmp in c("RNA", "PRO")) {
    ...
    p <- p + geom_point(data = tab2p %>%
                        filter(fdr >= 0.05), 
                        mapping = aes(x = source_genesymbol, y = meddiff), color = "grey50", alpha = 0.4, position = pos)
    p <- p + geom_point(data = tab2p %>%
                          filter(fdr < 0.05) %>%
                          filter(meddiff > 0), 
                        mapping = aes(x = source_genesymbol, y = meddiff), color = "red", position = pos)
    p <- p + geom_text_repel(data = tab2p %>% 
                               filter(fdr < 0.05) %>%
                               filter(meddiff > 0), 
                             mapping = aes(x = source_genesymbol, y = meddiff, label = SUB_GENE, alpha = point_alpha), color = "red", position = pos)
    p <- p + geom_point(data = tab2p %>%
                          filter(fdr < 0.05) %>%
                          filter(meddiff < 0), 
                        mapping = aes(x = source_genesymbol, y = meddiff), color = "blue", alpha = 0.8, position = pos)
    p <- p + geom_text_repel(data = tab2p %>% 
                               filter(fdr < 0.05) %>%
                               filter(meddiff < 0), 
                             mapping = aes(x = source_genesymbol, y = meddiff, label = SUB_GENE, alpha = point_alpha), color = "blue", position = pos)
    p = p + theme_nogrid()
    p <- p + xlab(paste0("Transcriptional Factor")) + ylab(paste0(expression_type_tmp, " level difference\nMedian(GOF Missense) - Median(other Missense)"))
    p <- p + guides(alpha = F)
    p
