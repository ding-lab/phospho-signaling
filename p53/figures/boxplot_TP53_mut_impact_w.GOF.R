...
# enzyme <- "TP53"; substrate <- "CDH3"; rsd <- "PRO"; enzyme_upstream <-  NULL
# enzyme <- "TP53"; substrate <- "TP53BP2"; rsd <- "PRO"; enzyme_upstream <-  NULL
# enzyme <- "TP53"; substrate <- "MCM7"; rsd <- "PRO"; enzyme_upstream <-  NULL
# enzyme <- "TP53"; substrate <- "RB1"; rsd <- "PRO"; enzyme_upstream <-  NULL
# enzyme <- "TP53"; substrate <- "AR"; rsd <- "PRO"; enzyme_upstream <-  NULL
# enzyme <- "TP53"; substrate <- "AR"; rsd <- "PRO"; enzyme_upstream <-  NULL
# enzyme <- "TP53"; substrate <- "FBP1"; rsd <- "PRO"; enzyme_upstream <-  NULL
# enzyme <- "TP53"; substrate <- "KRT18"; rsd <- "PRO"; enzyme_upstream <-  NULL
enzyme <- "TP53"; substrate <- "CDK1"; rsd <- "PRO"; enzyme_upstream <-  NULL
enzyme <- "TP53"; substrate <- "CDK1"; rsd <- "RNA"; enzyme_upstream <-  NULL
enzyme <- "TP53"; substrate <- "ECT2"; rsd <- "PRO"; enzyme_upstream <-  NULL
enzyme <- "TP53"; substrate <- "BRCA2"; rsd <- "RNA"; enzyme_upstream <-  NULL
...
for (cancer in c("OV")) {
  ...
  sup_tab$group <- NULL
  sup_tab <- merge(sup_tab, part_tab %>%
                     select(V1, Classification_complex),
                   by.x = c("partID"), by.y = c("V1"), all.x = T)
  sup_tab$group <- sup_tab$Classification_complex
  sup_tab <- sup_tab %>%
    filter(!is.na(group))
  ...
  tab2p$x <- factor(tab2p$x, levels = names(colors_mut_v2))
  ## filtering
  # tab2p <- tab2p[tab2p$x %in% names(colors_mut),]
  tab2p$y <- remove_outliers_IQR(x = tab2p$sub_exp, out_thres = 3, na.rm = T)
  if (cancer == "BRCA") {
    tab2p$subtype <- partID2pam50(patientID_vector = tab2p$partID, pam50_map = loadPAM50Map())
    tab2p$subtype_sim <- tab2p$subtype
    tab2p$subtype_sim[tab2p$subtype_sim %in% c("LumA", "LumB")] <- "Luminal"
    tab2p$subtype_sim[tab2p$subtype_sim %in% c("no_RNA-seq", "Normal")] <- "Other"
  } else {
    tab2p$subtype_sim <- ""
  }
  ...
  if (cohort == "CPTAC") {
    ...
    # p = p + geom_violin(aes(fill = x),  color = NA, alpha = 0.8)
    p = p + geom_boxplot(aes(fill = x), width=.2, alpha = 0.5, outlier.shape = NA)
    # p = p + geom_boxplot(width=.2, alpha = 0.5)
    p = p + geom_point(aes(shape = subtype_sim), color = "black", position = pos, stroke = 0, alpha = 0.6, size = 1.5)
    # p <- p + geom_text_repel(data = tab2p %>% 
    #                            ilter(group == "Missense_protein_level_high"), 
    #                          mapping = aes(x = x, y = y, label = SUB_GENE, alpha = point_alpha), color = "red", position = pos)
    p = p + scale_fill_manual(values = colors_mut_v2, name = "TP53 mutation status")
    # p = p + scale_color_manual(values = colors_mut_v2, name = "TP53 mutation status")
    if (rsd == "PRO") {
