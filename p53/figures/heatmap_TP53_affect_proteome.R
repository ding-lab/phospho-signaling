...
pairs2plot <- c("TP53:ESR1:PRO")
mut_genes <- c("TP53", "ESR1")
cna_genes <- c("TP53")
rna_genes <- c("TP53", "ESR1")
pro_genes <-  c("TP53", "ESR1")
pho_genes <- c("")
rsds <- c("")
phog_genes <- c("")
...
for (pair in pairs2plot) {
  ...
  for (cancer in c( "BRCA")) {
    ...
    if (!file.exists(fn)) {
      ...
      if (nrow(vaf_part) > 0) {
        col_anno <- merge(col_anno, vaf_part[, c("partID", "VAF")], by = c("partID"), all.x = T)
      } else {
        stop("no VAF!")
        col_anno$VAF <- NA
      }
      ...
      plotpheatmap(mat_value, color.palette, col_anno, ann_colors, 10, 10)
