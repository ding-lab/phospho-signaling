  ...
  for (variant_class in variant_classes2process) {
    for (cancer in cancers2process) {
      ...
      if (!file.exists(fn)) {
        ...
        pair_tab <- data.frame(GENE = "TP53", SUB_GENE = unique(affected_exp_head$SUBSTRATE))
  ...
  for (variant_class in c("truncation", "missense", "not_silent")) {
    for (cancer in c("BRCA", "OV", "CO", "UCEC", "CCRCC", "LIHC")) {
      for (genoalt_type in c("mut")) {
        fn <- paste0("./cptac2p/analysis_results/p53/tables/test_TP53_mut_impact_global_RNA_PRO_PHO/",  cancer, "_", variant_class, "_", gene_altered, "_mut_impact_", affected_exp_type , "_tab.txt")
        if (file.exists(fn)) {
...
saveRDS(object = mut_cnv_cans, file = paste0(makeOutDir(resultD = resultD), gene_altered, "_mut_impact_proteome_RNA_cptac2p_cptac3_tab.RDS"))
