...
# hotpho <- read_delim("~/Box Sync/Ding_Lab/Projects_Current/hotpho_data/data/Data_201807_cc.p0.05.cluster_transcriptSynced_hybrid.tsv", 
#                      "\t", escape_double = FALSE, col_types = cols(Cluster = col_character()), trim_ws = TRUE)
nrow(hotpho)
...
hotpho_cptac_m <- hotpho_cptac_m[!grepl(pattern = "Retrospective", x = hotpho_cptac_m$Cohort),]
...
tab4plot_sorted$Cohort <- order_cancer(x = tab4plot_sorted$Cohort)
...
for (gene in genes2test) {
  ...
  for (cohort in unique(hotpho_cptac_m_g$Cohort)) {
    ...
    if (cohort != "LUAD") {
      ...
      pho <- pho[pho$Gene == gene,]
      pho$rsd <- toupper(str_split_fixed(string = pho$Phosphosite, pattern = ":", 2)[,2])
      samples <- colnames(pho); samples <- samples[!(samples %in% c("Gene", "Phosphosite", "rsd"))]
      for (rsd in unique(hotpho_cptac_m_g$rsd[hotpho_cptac_m_g$Cohort == cohort])) {
        # rsd <- "S681"
        pho_rsd <- pho[grepl(pattern = rsd, x = pho$Phosphosite, ignore.case = T), samples]
        if (nrow(pho_rsd) == 0) {
...
p <- p + coord_flip()
p
fn = paste0(makeOutDir(resultD = resultD), "selected_genes_sample_sd.pdf")
ggsave(file=fn, height=5, width=3.5, useDingbats=FALSE)
