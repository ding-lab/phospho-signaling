...
cancer <- "CO"
exps <- fread(input = paste0("./MSI_CPTAC/Data/proteomics_level1/colon_global_with_genes.txt"), data.table = F)
exps <- exps[exps$hgnc_symbol %in% mmr_gene, c("hgnc_symbol", colnames(exps)[grepl("CO", x = colnames(exps)) & !grepl("N", x = colnames(exps))])]
exps <- melt(exps)
colnames(exps) <- c("Gene", "variable", "score")
## transform sample IDs to patient IDs
exps$Participant.ID <- substr(x = exps$variable, start = 2, stop = 8)
library(ggpubr)
## merge exps with MSI score
msi_exps <- merge(msi_score, exps, by.x = c("Sample"), by.y = c("Participant.ID"))
msi_exps$MSI_status <- ifelse(msi_exps$Score >= msi_thres, paste0("MSI-H_tumors"), paste0("other_tumors"))
resultDnow <- makeOutDir()
subdir <- paste0(resultDnow, cancer, "_level1")
dir.create(path = subdir)
## MLH1 not in colorectal proteomics
mann_results[[cancer]] <- vector("list")
for (gene in mmr_gene) {
  exps_gene <- msi_exps[msi_exps$Gene == gene & !is.na(msi_exps$score),]
  ...
  if (length(exps_msih) > 4) {
    ...
    p <- p + ylab("protein abundance (log2 ratio normalized)")
    p <- p + annotate("text", x = 1, y = 2 , label = paste0("Mann-Whitney, p = ", signif(stat$p.value, digits = 2)))
    p <- p + ggtitle(label = paste0(gene, " protein abundance between MSI-H tumor(N=", length(exps_msih),") and other tumors(N=", length(exps_msil), ")"))
    p <- p + theme_nogrid()
    p <- p + theme(axis.title.x = element_blank()) + theme(legend.position = "none") + theme(plot.title = element_text(size=8))
    p
    fn = paste0(subdir, gene, '_protein_abundance_between_MSI-H_and_rest_in_', cancer, ".pdf")
...
p <- ggboxplot(msi_exps, x = "Gene", y = "score",
               color = "MSI_status", palette = "jco",
               add = "jitter", xlab = "", ylab = "protein abundance (log2 ratio normalized)", font.label = list(size = 5, style = "plain") )
p + stat_compare_means(aes(group = MSI_status), label = "p.signif")
resultDnow <- makeOutDir()
fn = paste0(resultDnow, cancer, '_MMR_gene_protein_abundance_between_MSI-H_and_rest_in_', cancer, ".pdf")
...
for (gene in mmr_gene) {
  ...
  if (length(exps_msih) > 4) {
    ...
    # p <- p + theme(axis.title.x = element_blank()) + theme(legend.position = "none") + theme(plot.title = element_text(size=8))
    # p
    # fn = paste0(resultDnow, cancer, "/", gene, '_protein_abundance_between_MSI-H_and_rest_in_', cancer, ".pdf")
    # ggsave(file=fn, height=4, width=5, useDingbats=FALSE)
  } else {
    print(paste0(gene, " not enough data!"))
  }
