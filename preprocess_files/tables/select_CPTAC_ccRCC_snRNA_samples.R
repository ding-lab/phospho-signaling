...
for (cancer in cancers2process) {
  ...
  if (!file.exists(fn)) {
    ...
    estimate_tab <- readxl::read_excel("./Ding_Lab/Projects_Current/CPTAC/CPTACIII/CCRCC_AWG/CCRCC_shared_data/manuscripts/ccRCC Manuscript/CPTAC3-ccRCC-SupplementaryTables_Final/Table S7.xlsx", sheet = "ESTIMATE scores")
  ...
  # annotate the received 65 ccRCC samples ----------------------------------
    received_sample_tab <- readxl::read_excel("./Ding_Lab/Projects_Current/ccRCC_drug/resources/sample_info/CPTAC3_ccRCC_request/JHU Distribution Manifest_ccRCC material to WUSTL_6.17.2019.xlsx", range = "A1:I66")
    received_sample_tab <- merge(received_sample_tab, estimate_purity_rna_df %>%
                                   select(partID, ESTIMATE_TumorPurity_RNA), by.x = c("Subject ID"), by.y = c("partID"), all.x = T)
    received_sample_tab_fn <- paste0(makeOutDir(resultD = resultD), "JHU Distribution Manifest_ccRCC material to WUSTL_6.17.2019_annotated_v1.csv")
    write.table(x = received_sample_tab, file = received_sample_tab_fn, sep = ",", row.names = F, col.names = T, quote = F)
