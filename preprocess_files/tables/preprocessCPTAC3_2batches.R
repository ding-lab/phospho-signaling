...
base_ucec_pnnl <- paste(inputD, "7_CPTAC3_Uterine_Corpus_Endometrial_Carcinoma", sep = "")
raw["UCEC","protein"] <- paste(base_ucec_pnnl,"/UCEC_batch1-2_SummaryReports/CPTAC3_Uterine_Corpus_Endometrial_Carcinoma_Proteome.tmt10.tsv", sep = "")
raw["UCEC", "phosphoprotein"] <- paste(base_ucec_pnnl,"/UCEC_batch1-2_SummaryReports/CPTAC3_Uterine_Corpus_Endometrial_Carcinoma_Phosphoproteome.phosphosite.tmt10.tsv", sep = "")

for (cancer in c("UCEC")) {
  sink(paste0(inputD, cancer, "/", 'preprocessing_log.txt'), append=FALSE, split=FALSE)
  ...
  ## write out tables
  write.table(Pro.f2p, row.names = T, quote=F, sep = '\t', file=paste(inputD, cancer, "/", prefix[cancer], "_PRO_formatted.txt",sep=""))
...
base_CCRCC_jhu <- paste(inputD, "CCRCC", sep = "")
...
for (cancer in c("CCRCC")) {
  ...
  ## write out tables
  write.table(Pro.f2p, row.names = F, quote=F, sep = '\t', file=paste(inputD, cancer, "/", prefix[cancer], "_PRO_formatted.txt",sep=""))
  write.table(Pro.n2p, row.names = F, quote=F, sep = '\t', file=paste(inputD, cancer, "/", prefix[cancer], "_PRO_formatted_normalized.txt",sep=""))
  write.table(Pro.n_aveRep2p, row.names = F, quote=F, sep = '\t', file=paste(inputD, cancer, "/", prefix[cancer], "_PRO_formatted_normalized_replicate_averaged.txt",sep=""))
  write.table(Pro.n_tumor2p, row.names = F, quote=F, sep = '\t', file=paste(inputD, cancer, "/", prefix[cancer], "_PRO_formatted_normalized_noControl.txt",sep=""))
  write.table(Pro.n_normal2p, row.names = F, quote=F, sep = '\t', file=paste(inputD, cancer, "/", prefix[cancer], "_PRO_formatted_normalized_Control.txt",sep=""))
...
write_delim(clinical, delim = "\t", paste0(inputD, "Specimen_Data_20161005_Yige_20180307.txt"))
