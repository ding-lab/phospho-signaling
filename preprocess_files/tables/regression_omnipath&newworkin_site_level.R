...
inputnames <- c("_noControl", "")
names(inputnames) <- c("tumor", "normal+tumor")
...
ptms_site_pairs_sup <- merge(ptms_site_pairs_sup, op_enzyme, all.x = TRUE)
ptms_site_pairs_sup$enzyme_type[is.na(ptms_site_pairs_sup$enzyme_type)] <- "kinase"

# loop -----------------------------------------------------------------
for (sample_type in c("tumor", "normal+tumor")) {
  for( enzyme_type in c("kinase", "phosphotase")) {
    trans_table <- ptms_site_pairs_sup[ptms_site_pairs_sup$GENE %in% op_enzyme$GENE[op_enzyme$enzyme_type == enzyme_type],]
    trans_table <- trans_table[as.vector(trans_table$GENE)!=as.vector(trans_table$SUB_GENE),]
    cis_table <- load_ks_table(protein = enzyme_type)
    cis_table <- cis_table[as.vector(cis_table$GENE)==as.vector(cis_table$SUB_GENE),]

    table_4can <- NULL
    for (cancer in c("BRCA", "OV","CO", "UCEC")) { 
      pro_data <- fread(input = paste(inputD, cancer,"/",prefix[cancer], "_PRO_formatted_normalized", inputnames[sample_type], ".txt",sep=""))
      pho_data <- fread(input = paste(inputD, cancer,"/",prefix[cancer], "_PHO_formatted_normalized", inputnames[sample_type], ".txt",sep=""))
      pho_gdata <- fread(input = paste(inputD, cancer,"/",prefix[cancer], "_collapsed_PHO_formatted_normalized_replicate_averaged", ifelse(sample_type == "tumor", "_Tumor", ""), ".txt",sep=""))
      ...
      table$Cancer <- cancer
      table$SELF <- ifelse(as.character(table$KINASE) == as.character(table$SUBSTRATE), "cis", "trans")
      ...
      for(SELF in c("cis", "trans")) {
        for(coln in name) {#adjust pvalues for each variable
          row <- (table$SELF==SELF) & (table$Cancer==cancer)
          table[row,paste("FDR_",coln,sep = "")] <-p.adjust(table[row,paste("P_",coln,sep = "")],method = "fdr")
        }
      }
      
      ## write out
      tn = paste0(makeOutDir(), enzyme_type,"_substrate_regression_cptac2p_", cancer, "_", sample_type, ".txt")
      write.table(table, file=tn, quote=F, sep = '\t', row.names = FALSE)
      
      table_4can <- rbindlist(l = list(table_4can, table))
    }
    tn = paste0(makeOutDir(), enzyme_type,"_substrate_regression_cptac2p_4can_", sample_type, ".txt")
    write.table(table_4can, file=tn, quote=F, sep = '\t', row.names = FALSE)
  }
}
