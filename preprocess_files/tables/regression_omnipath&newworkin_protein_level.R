...
inputnames <- c("_noControl", "_replicate_averaged")
...
for (sample_type in c("tumor", "normal+tumor")) {
  ...
  for( enzyme_type in c("kinase", "phosphotase")) {
    k_s_table <- ptms_site_pairs_sup[ptms_site_pairs_sup$GENE %in% op_enzyme$GENE[op_enzyme$enzyme_type == enzyme_type],]
    ...
    for (cancer in c("BRCA","OV","CO", "UCEC")) { 
      pro_data <- fread(input = paste(inputD, cancer,"/",prefix[cancer], "_PRO_formatted_normalized", inputnames[sample_type], ".txt",sep=""), data.table = F)
      pho_data <- fread(input = paste(inputD, cancer,"/",prefix[cancer], "_PHO_formatted_normalized", inputnames[sample_type], ".txt",sep=""), data.table = F)
      pho_gdata <- fread(input = paste(inputD, cancer,"/",prefix[cancer], "_collapsed_PHO_formatted_normalized_replicate_averaged", ifelse(sample_type == "tumor", "_Tumor", ""), ".txt",sep=""), data.table = F)
      ...
      for (kinase in kinase_trans){
        ...
        if ( nrow(pho_kinase_g) > 0 ){
          ...
          k_sub <- k_sub[k_sub %in% pho_rsd_split$SUBSTRATE]
