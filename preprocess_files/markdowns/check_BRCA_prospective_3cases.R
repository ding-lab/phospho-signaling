...
# check 3 cases -----------------------------------------------------------
partIDs3cases <- c("05BR026", "01BR030", "20BR006")
## mutation
maf_3cases <- maf %>%
  mutate(partID = str_split_fixed(string = Tumor_Sample_Barcode, pattern = "_", 2)[,1]) %>%
  filter(partID %in% partIDs3cases)
  
maf_3cases_smg <- maf_3cases %>%
  filter(Hugo_Symbol %in% SMGs[["BRCA"]])

## subtype
partID2pam50(patientID_vector = partIDs3cases, pam50_map = loadPAM50Map())
